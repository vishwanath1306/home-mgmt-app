...
type UserType = "Vishwa" | "Shruthi"
type ViewModeType = "My Meals" | "Partner's Meals" | "Our Meals"
type MealType = "breakfast" | "lunch" | "dinner" | "snack"

/**
 * Meals Page Component
 * 
 * Comprehensive meal planning and nutrition tracking interface that provides:
 * - Weekly meal overview with expandable daily views
 * - Nutrition goal tracking with progress bars
 * - Water intake monitoring
 * - Quick action buttons for adding meals and water
 * - User switching between Vishwa and Shruthi
 * - View filtering (My Meals, Partner's Meals, Our Meals)
 */
export default function MealsPage() {
  // Destructure meal-related data and functions from the custom hook
  const { 
    meals, 
    nutritionGoals, 
    waterIntake, 
  ...
  const [currentUser, setCurrentUser] = useState<UserType>("Vishwa")
  const [viewMode, setViewMode] = useState<ViewModeType>("Our Meals")
  ...
  /**
   * Filter meals based on the selected view mode and user
   */
    ...
    switch (viewMode) {
      case "My Meals":
        return dateMeals.filter(meal => 
          meal.assignedTo === currentUser || meal.createdBy === currentUser
        )
      case "Partner's Meals":
        const partner = currentUser === "Vishwa" ? "Shruthi" : "Vishwa"
        return dateMeals.filter(meal => 
          meal.assignedTo === partner || meal.createdBy === partner
        )
      default:
        return dateMeals
    }
  ...
  // Get nutrition goals and calculate daily progress for current user
  ...
  // Calculate water intake for current user today
  ...
  /**
   * Add water intake for current user
   */
  ...
  /**
   * Open meal modal with pre-selected date and meal type
   */
  const openMealModal = (date: string, mealType: MealType) => {
  ...
  /**
   * Toggle day expansion in weekly view
   */
  ...
  const getMealTypeIcon = (mealType: string) => {
    ...
    return iconMap[mealType as MealType] || Utensils
  }
    ...
    <div className="w-full mx-auto bg-slate-50 min-h-screen flex flex-col">
      ...
      <div className="flex-1 p-4 pb-20 overflow-y-auto max-w-7xl mx-auto w-full">
        <div className="space-y-6">
          ...
          {/* Nutrition Progress for Current User */}
          {userNutritionGoals && (
            <Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              {/* View Mode Toggle */}
              <div className="flex gap-2">
                {(["My Meals", "Partner's Meals", "Our Meals"] as ViewModeType[]).map((mode) => (
                  <Button
                    key={mode}
